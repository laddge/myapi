...
def post(data):
    ...
    try:
        api = tweepy.API(auth)
        user = api.get_user(data['username'])
        ...
        bg = Image.open(os.path.join(os.path.dirname(__file__), "{}.png".format(data['waku'])))
        bg = bg.resize(size=(size, size))
