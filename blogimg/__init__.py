...
def get(text, bgcolor='(255, 255, 255)', fgcolor='(0, 0, 0)'):
    ...
    fgcolor = tuple([int(s.strip('()')) for s in fgcolor.split(',')])
    tl = text.strip().split('\n')
    ...
    draw.text(((640 - w) / 2, (ih - h - 60) / 2), text, fill=fgcolor, font=font)
    r = 20
    lw = 5
    m = 7
    lm = m + lw / 2
    draw.line((lm, lm + r - 1, lm, ih - lm - r + 1), fill=fgcolor, width=lw)
    draw.line((lm + r - 1, lm, 640 - lm - r + 1, lm), fill=fgcolor, width=lw)
    draw.line((640 - lm, lm + r - 1, 640 - lm, ih - lm - r + 1), fill=fgcolor, width=lw)
    draw.line((lm + r - 1, ih - lm, 640 - lm - r + 1, ih - lm), fill=fgcolor, width=lw)
    draw.arc((m, m, m + r * 2, m + r * 2), start=180, end=270, fill=fgcolor, width=lw)
    draw.arc(
